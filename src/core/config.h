...
struct GPSConfig {
    ...
    // CUSTOM so pin auto-select (GROVE→1/2) never clobbers the Pancake pins.
    // ESP RX=14 (module TX), ESP TX=13 (module RX) — ESP32Marauder MARAUDER_PANCAKE.
    GPSSource source = GPSSource::CUSTOM;
    uint8_t rxPin = 14;
    uint8_t txPin = 13;
#else
...
struct PersonalityConfig {
    ...
    uint8_t themeIndex = 15;            // Default: Realistic (REALISTIC_THEME_INDEX in display.h; 0-14 are the mono themes)
    G0Action g0Action = G0Action::IDLE;
